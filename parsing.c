...
lval *builtin_load(lenv *e, lval *v) {
    ...
    mpc_result_t r;
    if (mpc_parse_contents(v->cell[0]->str, Orelisp, &r)) {

        /* read contents */
        lval *expr = lval_read(r.output);
        mpc_ast_delete(r.output);

        /* evaluate reach expression */
        while (expr->count) {
        ...
        /* delete expresions and arguments */
        lval_del(expr);
        lval_del(v);
...
lval *builtin_lambda(lenv *e, lval *v) {
    ...
    for (int i = 0; i < v->cell[0]->count; i++) {
        LASSERT(v, (v->cell[0]->cell[i]->type == LVAL_SYM),
                "Cannot defne non-symbol. Got %s, Expected %s",
...
lval *lval_call(lenv *e, lval *f, lval *v) {
    /* simply apply if lval is builtin */
    ...
    while (v->count) {
        ...
        /* bind a copy into the functin's environment */
    ...
    /* argumetn list is now bound so can be cleaned up */
    lval_del(v);

    /* if all formals have been bound evaluate */
    if (f->formals->count == 0) {

        /* set environment parent to evaluation environemnt */
        f->env->par = e;
...
int main(int argc, char **argv) {
    ...
    "                                                                   \
        ...
        orelisp  : /^/ <expr>* /$/ ;                                    \
    ",
    Number, Symbol, String, Comment, Sexpr, Qexpr, Expr, Orelisp);

    lenv *e = lenv_new();
    lenv_add_builtins(e);

    if (argc == 1) {
        puts("Orelisp version 0.0.4");
    ...
    mpc_cleanup(8, Number, Symbol, String, Comment, Sexpr, Qexpr, Expr, Orelisp);
